...
def processData(imageJsonPath, labelJsonPath, saveDir):
  ...
  for labelIndex, label in enumerate(labelJson):
    ...
    land_mark = np.zeros([11, 2])
    disease_c = np.zeros([11, 5])
    for point in points:
    ...
    # land_mark = np.reshape(land_mark, [-1, 22])
    ...
    for image in images:
      ...
      if image['seriesUid'] == seriesUid:
        # 读取到和标注seriesUid相对应的图像时，
        # 1将图像添加到图像list中，
        # 2像素标注添加到land_marks中，
        # 3对应的分类添加disease_class中
        img_x = dicom2array(image['path'])
        imageData.append(img_x)
        land_marks.append(land_mark)
        disease_class.append(disease_c)
